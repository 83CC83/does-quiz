...
const DIMENSIONS = {
  D: {
    ...
    levels: {
      ...
      high: "你的腦子很少真正安靜下來。一句話、一個決定、一次對話，都可能在你心裡展開成一張複雜的網。你可能試過叫自己別想太多，但那個念頭本身又變成另一個值得想的事。睡前是最難的時候——因為沒有其他事情可以分散注意力了。",
    },
  },
  O: {
    ...
    levels: {
      ...
      high: "你的耗損速度比別人快，而且有時候你自己也沒發現。吵雜的餐廳、塞滿行程的週末、被打斷了好幾次的工作日——結束之後那種疲憊會壓過來，連思考都變得模糊。你可能試過告訴自己撐一下就好，但離開那個環境之後，才意識到自己一直繃著。回到一個人的空間，是你一天裡最需要的一刻。",
    },
  },
  E: {
    ...
    levels: {
      ...
      mid: "你有同理心，但你能控制它進來多少。別人的難過你感受得到，但你通常知道什麼時候該抽身，不會一直帶著別人的情緒走。",
  ...
  S: {
    ...
    levels: {
      low: "感官刺激對你的影響相對小。光線、聲音、氣味，你能注意到但不太困擾，在各種環境下都能保持狀態。",
      mid: "你對感官細節有一定的感知力。某些環境會讓你不太舒服，但你通常能找到辦法調整，不會被它佔據太多注意力。",
      high: "你的感官一直都是開著的。別人沒注意到的氣味、背景音樂的音量、衣服領口的觸感——這些對你來說不是小事，是真實存在的干擾。你可能試過跟自己說「沒什麼大不了」，但那個不舒服就是在那裡，很難靠意志力讓它消失。有時候一個環境好不好，你走進去的瞬間就知道了。",
    },
  },
  HSS: {
    ...
    levels: {
      ...
      high: "你同時住著兩個互相拉扯的自己。一個需要安靜、需要恢復、需要不被打擾；另一個悶了就想出走，看到沒去過的地方就想進去，熟悉的事情做久了就開始覺得哪裡不對。你可能很難跟別人解釋這種矛盾——明明容易累，卻又靜不下來。",
    },
  },
};
...
const QUESTIONS = [
  ...
  { id: "D2", dim: "D", scenario: "朋友隨口分享了一件事，說完就換話題了。但你？", options: [{ label: "跟著換話題，沒特別放心上", value: 1 }, { label: "當下有多想一下，之後就忘了", value: 2 }, { label: "事後還會想起來，試著理解對方為什麼這樣說、當時的心情是什麼", value: 3 }, { label: "那句話變成一個入口——開始想到自己、想到某段關係、想到一些更大的問題，越想越深", value: 4 }] },
  ...
  { id: "O3", dim: "O", scenario: "一個週末你參加了整天的活動，人很多、行程很滿。結束後你？", options: [{ label: "覺得很充實，精力還很好", value: 1 }, { label: "有點累，睡一覺就恢復了", value: 2 }, { label: "結束當下就很想一個人待著，需要安靜的空間才能放鬆", value: 3 }, { label: "活動進行到一半就已經開始覺得撐不住，需要找藉口提早離開才能恢復", value: 4 }] },
  ...
  { id: "E2", dim: "E", scenario: "你在看一部電影，裡面有一個角色經歷了很大的失去。你通常？", options: [{ label: "覺得劇情很好，但情緒上沒有特別大的波動", value: 1 }, { label: "有點難過，但出了電影院就恢復了", value: 2 }, { label: "那種難過會持續一陣子，需要時間才能從角色的情緒裡抽出來", value: 3 }, { label: "看完之後會反覆想那個角色的處境，甚至替他感到不平或心疼", value: 4 }] },
  ...
  { id: "E4", dim: "E", scenario: "你身邊有人在吵架或關係緊張，即使不是針對你。你？", options: [{ label: "不太受影響，那是他們的事", value: 1 }, { label: "有感覺到氣氛不對，但可以切換注意力", value: 2 }, { label: "會不自覺地想緩和氣氛或做些什麼", value: 3 }, { label: "會忍不住想介入或離開現場，那種緊張感讓你無法假裝沒事", value: 4 }] },
  ...
  { id: "S1", dim: "S", scenario: "你走進一個剛打掃過、充滿清潔劑味道的房間。你？", options: [{ label: "沒特別注意到味道，繼續做自己的事", value: 1 }, { label: "有聞到，但幾分鐘後就適應了", value: 2 }, { label: "味道讓你有點不舒服，需要開窗或離開一下", value: 3 }, { label: "那個味道會持續干擾你，很難在那個空間裡專心做事", value: 4 }] },
  ...
  { id: "HSS4", dim: "HSS", scenario: "你在一個已經很熟悉的領域被交付一個全新的、有點冒險的專案。你？", options: [{ label: "有點擔心，希望有更多資訊和把握再開始", value: 1 }, { label: "接受了，但會盡量在熟悉的框架內進行", value: 2 }, { label: "覺得是個好機會，喜歡嘗試沒做過的事", value: 3 }, { label: "比起熟悉的專案，這種有點冒險的反而讓你更專注、更有創意", value: 4 }] },
...
export default function DOESQuiz() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#fdf6f8", fontFamily: "'Zen Maru Gothic', 'Noto Sans TC', sans-serif", display: "flex", flexDirection: "column", alignItems: "center", padding: "0 22px" }}>
      ...
      <div style={{ width: "100%", maxWidth: 560, paddingBottom: 72 }}>
